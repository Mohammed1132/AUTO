...
const name = info[event.senderID].name
    ...
    if(msgData[event.messageID].attachments.length === 0) {
        api.sendMessage(`${name} قام بحذف رسالة😳: ${msgData[event.messageID].body}`, event.threadID)  
      ...
      for (const item of msgData[event.messageID].attachments) {
       let { data } = await axios.get(item.url, {responseType: "arraybuffer"})
          ...
          del.push(`./script/cache/${item.filename}.jpg`)
                    ...
                    api.sendMessage({body:`${name} قام بحذف صورة: ${msgData[event.messageID].body}`, attachment: photo}, event.threadID, () => {
...
api.sendMessage({body:`${name} قام بحذف هذا: ${msgData[event.messageID].body}`, attachment: fs.createReadStream('./script/cache/audio.mp3')}, event.threadID, () => {
